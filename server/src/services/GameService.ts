...
export class GameService {
  ...
  static async createRoom(roomId: string, width: number = 300, height: number = 1000): Promise<GameRoom> {
  ...
  static async addPlayerToRoom(roomId: string, playerId: string, nickname: string): Promise<Player> {
  ...
  static async removePlayerFromRoom(roomId: string, playerId: string): Promise<void> {
  ...
  static async processWordAttempt(roomId: string, attempt: WordAttempt): Promise<{
    success: boolean;
    foundWord?: FoundWord;
    error?: string;
  }> {
    ...
    const validation = WordValidator.validateAttempt(attempt, room.board, room.words);
  ...
  private static async cacheRoomData(room: GameRoom): Promise<void> {
    try {
      await redisClient.setEx(`room:${room.id}`, 3600, JSON.stringify(room.toJSON())); // 1 hour TTL
    } catch (error) {
      console.error('Failed to cache room data:', error);
    }
  }

  private static async loadRoomFromCache(roomId: string): Promise<GameRoom | null> {
    try {
