...
export class GameController {
  ...
  private setupSocketHandlers(): void {
    this.io.on('connection', (socket: Socket) => {
      ...
      socket.on('join-room', async (roomId: string, playerData: { nickname: string }) => {
        try {
  ...
  private async handleJoinRoom(socket: Socket, roomId: string, playerData: { nickname: string }): Promise<void> {
    // Add player to room
    const player = await GameService.addPlayerToRoom(roomId, socket.id, playerData.nickname);
    ...
    socket.to(roomId).emit('ranking-updated', GameService.getRoomRanking(roomId));
    socket.to(roomId).emit('game-stats', GameService.getRoomStats(roomId));

    console.log(`User ${socket.id} (${playerData.nickname}) joined room ${roomId}`);
  }
  ...
  private async handleWordAttempt(socket: Socket, attempt: WordAttempt): Promise<void> {
    ...
    if (result.success && result.foundWord) {
      ...
      this.io.to(roomId).emit('ranking-updated', GameService.getRoomRanking(roomId));
      this.io.to(roomId).emit('game-stats', GameService.getRoomStats(roomId));
      
      console.log(`Word "${result.foundWord.word}" found by ${result.foundWord.playerNickname} in room ${roomId}`);
    } else {
