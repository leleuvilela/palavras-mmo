import type { LetterGrid, WordPlacement } from '../../../../shared/src/types.js';
...
export class BoardGenerator {
  ...
  static generate(width: number, height: number, words: string[]): { board: LetterGrid, placements: WordPlacement[] } {
    const board: string[][] = Array(height).fill(null).map(() => Array(width).fill(''));
  ...
  private static placeWord(board: string[][], word: string, width: number, height: number): WordPlacement | null {
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      if (this.canPlaceWord(board, word, startX, startY, direction.dx, direction.dy, width, height)) {
        this.placeWordOnBoard(board, word, startX, startY, direction.dx, direction.dy);
        return {
          word,
          startX,
          startY,
  ...
  private static fillEmptyCells(board: string[][], width: number, height: number): void {
  ...
  static generateWordList(count: number): string[] {
    ...
    const words = [
      'JAVASCRIPT', 'PYTHON', 'REACT', 'NODE', 'EXPRESS', 'MONGODB', 'MYSQL',
      'HTML', 'CSS', 'TYPESCRIPT', 'ANGULAR', 'VUE', 'SVELTE', 'NEXT',
      'WEBPACK', 'VITE', 'ESLINT', 'PRETTIER', 'JEST', 'CYPRESS',
      'DOCKER', 'KUBERNETES', 'AWS', 'AZURE', 'GCP', 'NGINX', 'REDIS',
      'POSTGRESQL', 'FIREBASE', 'GRAPHQL', 'REST', 'API', 'JWT', 'OAUTH',
      'GIT', 'GITHUB', 'GITLAB', 'BITBUCKET', 'CI', 'CD', 'DEVOPS',
      'ALGORITHM', 'DATABASE', 'FRONTEND', 'BACKEND', 'FULLSTACK',
      'RESPONSIVE', 'MOBILE', 'DESKTOP', 'WEB', 'APP', 'FRAMEWORK',
      'LIBRARY', 'PACKAGE', 'MODULE', 'COMPONENT', 'FUNCTION', 'CLASS',
      'OBJECT', 'ARRAY', 'STRING', 'NUMBER', 'BOOLEAN', 'VARIABLE'
    ];
