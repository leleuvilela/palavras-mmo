import type { WordAttempt, LetterGrid, WordPlacement } from '../../../../shared/src/types.js';
...
export class WordValidator {
  static validateAttempt(
    ...
    if (!this.isWithinBounds(startX, startY, endX, endY, board.width, board.height)) {
    ...
    const actualLetters = this.getLettersFromBoard(board, startX, startY, endX, endY);
    ...
    const matchingWord = this.findMatchingWord(validWords, startX, startY, endX, endY, actualLetters);
  ...
  private static isWithinBounds(startX: number, startY: number, endX: number, endY: number, width: number, height: number): boolean {
  ...
  private static isValidDirection(startX: number, startY: number, endX: number, endY: number): boolean {
  ...
  private static getLettersFromBoard(board: LetterGrid, startX: number, startY: number, endX: number, endY: number): string {
  ...
  private static findMatchingWord(
    ...
    return validWords.find(word => {
      // Check if positions match
      const wordEndX = this.getWordEndX(word);
      const wordEndY = this.getWordEndY(word);

      const forwardMatch = (word.startX === startX && word.startY === startY && 
                           wordEndX === endX && wordEndY === endY &&
                           word.word.toUpperCase() === letters);

      const backwardMatch = (word.startX === endX && word.startY === endY &&
                            wordEndX === startX && wordEndY === startY &&
