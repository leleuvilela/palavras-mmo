import type { GameRoom as GameRoomType, LetterGrid, WordPlacement, FoundWord, GameStats } from '../../../shared/src/types.js';
...
export class GameRoom implements Omit<GameRoomType, 'players'> {
  ...
  getStats(): GameStats {
    return {
      ...
      completionPercentage: Math.round((this.foundWords.length / this.words.length) * 100),
    };
  }
  ...
  getRanking() {
    return Array.from(this.players.values())
      .map((player, index) => ({
        ...
        score: player.score,
        wordsFound: this.foundWords.filter(w => w.playerId === player.id).length,
        rank: index + 1,
      }))
  ...
  isWordFound(word: string): boolean {
    return this.foundWords.some(fw => fw.word.toLowerCase() === word.toLowerCase());
  }
